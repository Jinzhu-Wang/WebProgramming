...
TimerQueue::~TimerQueue(){
    ...
    close(timerfd_);
    for (const auto &entry : timers_){ //因为set里面的每个元素都是const，它是基于键排序的容器
        delete entry.second;
    }
}
...
void TimerQueue::HandleRead(){
    ...
    auto end = timers_.lower_bound(Entry(TimeStamp::Now(), reinterpret_cast<Timer *>(UINTPTR_MAX)));//查找“第一个不小于给定键”的元素位置
