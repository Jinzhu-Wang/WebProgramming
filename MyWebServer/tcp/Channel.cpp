...
Channel::Channel(int fd, EventLoop* loop): loop_(loop), fd_(fd), listen_events_(0), ready_events_(0), in_epoll_(false){}
Channel::~Channel(){
    if(fd_ != -1){
        close(fd_);
        fd_ = -1;
...
void Channel::SetInEpoll(bool inpoll){ in_epoll_ = inpoll;}

void Channel::SetReadyEvents(int ev){ ready_events_ = ev;}

void Channel::set_read_callback(std::function<void()>const &callback){ read_callback_ = std::move(callback);} //因为前面const的限制，实际为拷贝
