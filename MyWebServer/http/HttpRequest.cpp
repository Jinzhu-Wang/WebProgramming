...
HttpRequest::HttpRequest() : method_(RequestMethod::kInvalid), version_(RequestVersion::kUnknown){};
HttpRequest::~HttpRequest(){};

void HttpRequest::SetVersion(const std::string & ver){
    if(ver == "1.0"){
        version_ = RequestVersion::kHttp10;
    }else if(ver == "1.1"){
        version_ = RequestVersion::kHttp11;
    }else{
        version_ = RequestVersion::kUnknown;
    }
}

RequestVersion HttpRequest::version() const{
    return version_;
}

std::string HttpRequest::GetVersionString() const{
    std::string ver;
    if (version_ == RequestVersion::kHttp10)
    {
        ver = "http1.0";
    }
    else if (version_ == RequestVersion::kHttp11){
        ver = "http1.1";
    }else{
        ver = "unknown";
    }
...
bool HttpRequest::SetMethod(const std::string &_method){
    if(_method == "GET"){
        method_ = RequestMethod::kGet;
    }else if(_method == "POST"){
        method_ =  RequestMethod::kPost;
    }else if(_method == "HEAD"){
        method_ = RequestMethod::kHead;
    }else if(_method == "PUT"){
        method_ = RequestMethod::kPut;
    }else if(_method == "Delete"){
        method_ = RequestMethod::kDelete;
    }else{
        method_ = RequestMethod::kInvalid;
    }
    return  method_ != RequestMethod::kInvalid;
}
...
std::string HttpRequest::GetMethodString() const{
    std::string _method;
    if (method_ == RequestMethod::kGet)
    {
        _method = "GET";
    }
    else if (method_ == RequestMethod::kPost)
    {
        _method = "POST";
    }
    else if (method_ == RequestMethod::kHead){
        _method =  "HEAD";
    }
    else if(method_ == RequestMethod::kPut){
        _method = "PUT";
    }
    else if (method_ == RequestMethod::kDelete)
    {
        _method =  "DELETE";
    }else{
        _method = "INVALID";
